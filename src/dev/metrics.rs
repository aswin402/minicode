...
pub fn sample_process_metrics(pid: u32) -> DevProcessResourceUsage {
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        // 2. Read CPU time from /proc/<pid>/stat
        let stat_path = format!("/proc/{}/stat", pid);
        if let Ok(content) = std::fs::read_to_string(&stat_path) {
            if let Some(r_paren) = content.rfind(')') {
                let remainder = &content[r_paren + 1..];
                let fields: Vec<&str> = remainder.split_whitespace().collect();
                // fields[11] is utime, fields[12] is stime (0-indexed from after comm)
                if fields.len() > 13 {
                    let utime: f32 = fields[11].parse().unwrap_or(0.0);
                    let stime: f32 = fields[12].parse().unwrap_or(0.0);
                    let total_jiffies = utime + stime;

                    // Approximate percentage based on total jiffies (clamped for stability)
                    let est_cpu = (total_jiffies / 100.0).clamp(0.0, 100.0);
