...
5. **Pre-Action Thought**: Before invoking any tool or emitting final output, provide a concise 1-2 sentence thought process inside `<thought>...</thought>` tags explaining your immediate intent.
6. **Action Over Verbosity**: Keep explanations minimal. Let verified code, diffs, and test outputs speak for themselves.
7. **Positive Error Handling**: Always propagate errors using the `?` operator or return `Result<T, MinicodeError>`. If unwrapping is tempting, use `.ok_or_else(|| ...)?`.
