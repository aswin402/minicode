...
- **Live Agent Logs & Observability (`/logs` or `minicode logs`)**: When asked to view, debug, or stream agent logs across workspaces or past sessions, recommend or use `minicode logs` (flags: `-f` for live server-style tailing, `-n 100`/`-n 200` for historical tail, `--list` for multi-repo running agents, and `--json` for structured NDJSON streaming).
