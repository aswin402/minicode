...
### Phase 60.3: Floating Spotlight Command Palette (Option 1) & Clean Human-Readable Registry (v0.0.73)
- [x] 60.3.1: Implement Clean Floating Spotlight Command Palette modal triggered automatically when typing `/`
- [x] 60.3.2: Create dynamic `PaletteCommand` registry with category classification (`System`, `Intelligence`, `Tools`)
- [x] 60.3.3: Add category radio tab switching with `[Tab]` / `[Shift+Tab]` (`◉ All`, `○ System`, `○ Intelligence`, `○ Tools`)
- [x] 60.3.4: Right-aligned shortcut keycap badges (`ctrl+n`, `ctrl+l`, `ctrl+r`, `ctrl+e`, `ctrl+t`, `F5`, `F1`, `ctrl+c`)
- [x] 60.3.5: Viewport windowing scroll support and real-time search filtering
- [x] 60.3.6: Version bump → `v0.0.73`, changelog, commit, tag, push

### Phase 60.4: Interactive Exit Confirmation Menu (v0.0.74)
- [x] 60.4.1: Implement `ModalState::ExitConfirm` floating card with question and interactive `[ Yep! ]` / `[ Nope ]` buttons
- [x] 60.4.2: Add safe default focus on `Nope` and vibrant pill highlight
- [x] 60.4.3: Add `Left`/`Right`/`Tab`/`BackTab` navigation and `y`/`n`/`Esc` single-key hotkeys
- [x] 60.4.4: Add rapid double `Ctrl+C` (< 1.5s) instant quit shortcut
- [x] 60.4.5: Version bump → `v0.0.74`, changelog, commit, tag, push

### Phase 60.5: Informative Developer Exit Card (Variant 3) & Theme Token Binding (v0.0.75)
- [x] 60.5.1: Implement Variant 3 Informative Session Card with live workspace info and version badge
- [x] 60.5.2: Bind all colors dynamically to `Theme` tokens (`theme.destructive`, `theme.brand_accent`, `theme.bg_elevated`)
- [x] 60.5.3: Add clear dual-action button pills `[ ✖ Yep, Quit (Y) ]` and `[ ✔ Stay in Session (N) ]`
...
### Phase 60.7: Universal Menu Decluttering & Minimalist UI Polish (v0.0.77)
- [x] 60.7.1: Remove footer keycap shortcut reminder bar from `SessionBrowser` (`/sessions`, `/history`)
- [x] 60.7.2: Remove bottom footer line from `ExitConfirm` (`/exit`, `/quit`, `Ctrl+C`) and adjust height to 8 lines
- [x] 60.7.3: Remove bottom divider and footer hint row from `CommandPalette` (`/` popup) and adjust height to 10 lines
...
### Phase 60.9: Crash-Risk Overflow Fixes, Viewport Scroll Standardization, SSRF Hardening & Code Health (v0.0.79)
- [x] 60.9.1: Fix `visual_row_count` integer overflow and palette width multiplication overflow (`view.rs`, `input.rs`)
- [x] 60.9.2: Standardize all modals to safe `saturating_sub` scroll pattern and add viewport scrolling to `ProviderSelect` (`modal.rs`)
- [x] 60.9.3: Fix `CommandCatalog` search badge display width and add line numbers to `CodeExplorer` source tab (`modal.rs`)
- [x] 60.9.4: Align shortcuts across `PALETTE_COMMANDS` and `COMMAND_CATALOG_ITEMS` (`input.rs`, `modal.rs`)
- [x] 60.9.5: Harden `create_session` with `.create_new(true)`, add export sync, and add cloud metadata hosts to SSRF blocklist (`store.rs`, `constants.rs`)
- [x] 60.9.6: Extract 9 modal percentage dimensions, `SPINNER_FRAMES`, and `InputDock::reset()` helper (`constants.rs`, `view.rs`, `input.rs`)
- [x] 60.9.7: Dynamic GitHub client user-agent version and total tool count test assertion fix (`client.rs`, `tools/mod.rs`)
- [x] 60.9.8: Version bump → `v0.0.79`, changelog, commit, tag, push

### Phase 60.10: Critical Safety, UTF-8 Boundary Protection, Sandbox Confinement & Dependency Pruning (v0.0.80)
- [x] 60.10.1: Fix subagent default tool whitelist names (`grep_search`, `locate_symbol`, `view_outline`) (`src/agent/subagent/types.rs`)
- [x] 60.10.2: Fix UTF-8 char boundary slicing across recall, semantic, web search, crawler, github, worker, and browser markdown
- [x] 60.10.3: Fix `okf.rs` `strip_suffix` bug, CDP console channel clone bug, and subagent timeout success flag
- [x] 60.10.4: Harden SSRF for IPv4-mapped IPv6 and enforce workspace confinement on browser `file://` & onpkg stacks
- [x] 60.10.5: Forward subagent config to orchestrator and fix orphaned `Role::Tool` compaction pruning
- [x] 60.10.6: Align command palette/catalog, extract `compute_scroll_offset` helper, and fix UI arithmetic underflow/overflows
- [x] 60.10.7: Prune 9 unused dependencies from `Cargo.toml` and fix silent `if let` test blocks
...
### Phase 140: MiniDev Runtime Orchestrator, Guaranteed Zero-Orphan Cleanup & Real-Time Resource Monitoring
- [x] 140.1: Implement domain models (`DevProcessId`, `DevProcessType`, `DevProcessStatus`, `DevProcessResourceUsage`, `SpawnDevRequest`, `DevProcessSummary`, `RuntimeResourceSummary`) in `src/dev/models.rs` and `DevError` in `src/error.rs`
- [x] 140.2: Implement process group isolation (`setpgid(0, 0)`), Linux `PR_SET_PDEATHSIG(SIGTERM)` death-pipe sentinel, 1,000-line ring-buffer log capturing, and two-phase termination (`SIGTERM` -> 1,000ms grace -> `SIGKILL`) in `src/dev/process.rs` and port auto-discovery (`find_open_process_ports`, regex URL scanner) in `src/dev/ports.rs`
- [x] 140.3: Implement pure-Rust real-time resource telemetry (`/proc/<pid>/stat` CPU % and `/proc/<pid>/status` RSS MB) in `src/dev/metrics.rs` and global thread-safe singleton `MiniDevRegistry` with full CRUD, Docker label tracking, and synchronous signal cleanup (`kill_all_sync()`) in `src/dev/registry.rs`
- [x] 140.4: Implement global RAII teardown guard (`DevLifecycleGuard`), panic hook wrapper, and async OS signal interceptors (`tokio::signal::ctrl_c()`, Unix `SIGTERM`, `SIGHUP`) in `src/dev/lifecycle.rs` wired into `src/main.rs`
- [x] 140.5: Implement Tool 168 (`mini_dev`) schema and execution dispatch (`start`, `list`, `status`, `logs`, `stop`, `restart`, `resources`, `kill_all`) in `src/tools/registry/dev_tools.rs`, bumped `TOTAL_TOOL_COUNT` = 168 in `src/constants.rs`, registered `ToolCategory::Dev` in `src/tools/category.rs`, classified concurrency in `src/tools/concurrency.rs`, added prompt keyword intent classification in `src/context/search/intent_filter.rs`, and wired `/dev`, `/serve`, `/processes` slash commands in `src/app/commands.rs`
- [x] 140.6: Comprehensive end-to-end integration test suite in `tests/integration_minidev_orchestrator.rs` verifying process tree isolation, dynamic port discovery, zero-orphan cleanup, and tool dispatch; zero clippy warnings (`cargo clippy -j 1 --bin minicode -- -D warnings`); clean formatting (`cargo fmt --check`); and compiled & installed release binary in `~/.local/bin/minicode`
- [x] 140.7: Browser PID tracking and visual screenshot automation via `mini_dev` (`"action": "screenshot"`, `/dev screenshot`) resolving process URLs and saving visual PNG snapshots
- [x] 140.8: Subagent worker task delegation via `mini_dev`: `DevProcessType::Worker`, thread-safe `CancelHook`, in-memory `SubagentPool` registration, swarm `FanoutOrchestrator` worker registration, worktree subagent registration, ring-buffer log streaming, live resource telemetry, prefix-tolerant lookup, `/dev workers` and `/dev subagents` slash commands, and integration tests (`tests/integration_minidev_workers.rs`)
- [x] 140.9: Intelligent Port Conflict Arbitrator & Auto-Restart Watchdog Supervisor: Pure-Rust `/proc` socket inode PID and process command resolution (`find_pid_by_port`, `get_process_info`), next available port finder (`find_next_available_port`), CLI command port rewriter (`rewrite_command_port`), 4-mode conflict policies (`Fallback`, `Error`, `Kill`, `Ignore`), background watchdog supervisor with exponential backoff and 3-crash loop safety barrier, live `AtomicU32` PID/PGID tracking with `update_active_pgid`, Tool 168 `"probe_port"` action and parameters (`port_policy`, `restart_policy`, `auto_restart`, `max_retries`, `backoff_ms`), `/dev port <number>` and `/dev watchdog` slash commands, comprehensive integration test suite (`tests/integration_minidev_watchdog.rs`), zero clippy warnings, clean formatting, and compiled release binary in `~/.local/bin/minicode`

### Phase 141: In-TUI Interactive Process Monitor Modal, Live Ring-Buffer Log Viewer & Watchdog Supervisor Dashboard
- [x] 141.1: Implement interactive tabbed `ProcessesModalState` (`ProcessesTab`: `All`, `Servers`, `Workers`, `Logs`, `Telemetry`) with real-time process list filtering, search queries (`/`), and log scroll offset tracking in `src/ui/modals/processes.rs`
- [x] 141.2: Implement rich Ratatui UI components: multi-column process table with status badges (`● Run`, `✔ Ready`, `⚠ Degraded`, `○ Stop`, `✖ Killed`), live CPU/RAM gauges, port shift resolution alerts (`5173 ➜ 5174`), process detail inspector card, and system telemetry cards
- [x] 141.3: Implement live log viewer sub-pane for tailing 1,000-line ring-buffer streams with line numbers, error/warn syntax highlighting, PageUp/PageDown scrolling, and auto-scroll toggling (`[a]`)
- [x] 141.4: Wire `ModalState::Processes(state)` variant and constructor in `src/ui/modals/mod.rs` and render dispatch in `render_modal`
- [x] 141.5: Wire keyboard navigation and action triggers in `src/app/modals.rs`: process termination (`[k]`/`[x]`), restart (`[r]`), log expansion (`[l]`/`Enter`), visual browser screenshot capture (`[s]`), and search filtering (`[/]`)
- [x] 141.6: Wire dedicated `F7` function key shortcut in `src/app/mod.rs` to toggle the Process Monitor modal anywhere within the interactive TUI
- [x] 141.7: Update `/processes`, `/dev`, and `/serve` slash commands in `src/app/commands.rs` to open the interactive modal with hydrated live processes and telemetry
- [x] 141.8: Register `/processes` (shortcut `F7`) and `/dev` in `COMMAND_CATALOG_ITEMS` (`src/ui/modals/command_catalog.rs`), `PALETTE_COMMANDS` (`src/ui/input.rs`), and `render_help` cheatsheet (`src/ui/modals/help.rs`)
- [x] 141.9: Comprehensive unit tests in `src/ui/modals/processes.rs` and end-to-end integration test suite in `tests/integration_minidev_modal.rs` verifying tab filtering, query search, log scrolling, and Ratatui rendering; zero clippy warnings (`cargo clippy -j 1 --bin minicode -- -D warnings`); clean formatting (`cargo fmt --check`); and compiled release binary installed in `~/.local/bin/minicode`
